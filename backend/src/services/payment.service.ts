...
export type PaymentMethod = "bkash" | "nagad" | "rocket" | "upay" | "cash_on_delivery";
...
const defaultGateways: Array<{ name: string; gateway_code: PaymentMethod }> = [
...
const getActiveGateways = async () => {
  const existing = await db.query("SELECT COUNT(*)::int AS count FROM payment_gateways");
  ...
  return result.rows.map((row: any) => row.gateway_code);
};

const getFailedAttempts = async (orderId: string) => {
  const result = await db.query(
...
const recordFailedAttempt = async (orderId: string, userId: string) => {
  ...
  if (!existing.rows[0]) {
    await db.query(
      `INSERT INTO payment_attempts (order_id, user_id, failed_attempts, last_failed_at)
       VALUES ($1, $2, 1, NOW())`,
      [orderId, userId]
    );
    return 1;
  ...
  return updated.rows[0].failed_attempts as number;
};

const resetFailedAttempts = async (orderId: string) => {
  await db.query(
    `UPDATE payment_attempts
...
const isBlacklisted = async (phone?: string) => {
...
const computeRisk = (flags: string[]) => {
  let score = 0;

  if (flags.includes("duplicate_transaction")) score += 90;
  if (flags.includes("blacklisted_phone")) score += 90;
  if (flags.includes("too_many_failed")) score += 60;
  if (flags.includes("invalid_phone")) score += 50;
  if (flags.includes("invalid_transaction")) score += 40;
  if (flags.includes("amount_mismatch")) score += 40;
  if (flags.includes("gateway_inactive")) score += 30;

  if (score > 100) score = 100;

  let level: RiskResult["level"] = "low";
  if (score >= 71) level = "high";
  else if (score >= 31) level = "medium";

  return { score, level, flags } as RiskResult;
};

const markOrderRisk = async (orderId: string, risk: RiskResult) => {
  await db.query(
    `UPDATE orders
     SET risk_score = $1,
         risk_level = $2,
         ...
         updated_at = NOW()
     WHERE id = $4`,
    [risk.score, risk.level, risk.level === "high" || risk.flags.length > 0, orderId]
  );
};

export const submitPayment = async (userId: string, input: {
  order_id: string;
  ...
  if (failedAttempts >= 3) {
    ...
    throw new AppError("Too many failed attempts. Please contact support.", 429);
  }
  ...
  if (input.transaction_id) {
    const existingTxn = await db.query(
      `SELECT id FROM payment_logs WHERE transaction_id = $1
       UNION
  ...
  if (blockingFlags.length) {
    ...
    throw new AppError("Payment validation failed. Please review your details.", 400);
  }
...
export const listPayments = async (filters?: { status?: string; risk?: string }) => {
...
export const listPaymentLogs = async (orderId?: string) => {
  const result = await db.query(
    `SELECT id, order_id, customer_id, payment_method, sender_phone, transaction_id, amount,
...
export const getRiskReport = async (level?: string) => {
...
export const verifyPayment = async (orderId: string) => {
  const updated = await db.query(
    `UPDATE orders
     ...
     RETURNING id, order_number, payment_status, verification_status, status, user_id`,
    [orderId]
  );

  if (!updated.rows[0]) {
    throw new AppError("Order not found or not pending verification.", 404);
  }
...
export const rejectPayment = async (orderId: string) => {
  const updated = await db.query(
    `UPDATE orders
...
export const createDispute = async (userId: string, input: {
  order_id: string;
  payment_method: PaymentMethod;
  transaction_id: string;
  description: string;
  ...
  const order = await db.query("SELECT id, user_id FROM orders WHERE id = $1", [input.order_id]);
  if (!order.rows[0]) {
    throw new AppError("Order not found.", 404);
  }
...
export const listDisputes = async () => {
  const result = await db.query(
    `SELECT id, order_id, user_id, payment_method, transaction_id, description, screenshot_url, status, created_at, updated_at
...
export const getPaymentAnalytics = async (days = 30) => {
  ...
  const fraudReport = await db.query(
  ...
  return {
    daily_revenue: dailyRevenue.rows.map((row: any) => ({
      day: row.day,
      amount: Number(row.amount)
    })),
    monthly_revenue: monthlyRevenue.rows.map((row: any) => ({
      month: row.month,
      amount: Number(row.amount)
    })),
