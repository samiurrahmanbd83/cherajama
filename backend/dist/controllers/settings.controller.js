"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateWebsiteSettings = exports.getWebsiteSettings = void 0;
const AppError_1 = require("../utils/AppError");
const settings_service_1 = require("../services/settings.service");
// Public: fetch website settings
const getWebsiteSettings = async (_req, res) => {
    const settings = await (0, settings_service_1.getSettings)();
    res.status(200).json({ success: true, data: { settings } });
};
exports.getWebsiteSettings = getWebsiteSettings;
// Admin: update website settings
const updateWebsiteSettings = async (req, res) => {
    let socialLinks;
    if (typeof req.body.social_links === "string") {
        try {
            socialLinks = JSON.parse(req.body.social_links);
        }
        catch {
            throw new AppError_1.AppError("social_links must be valid JSON.", 400);
        }
    }
    else if (req.body.social_links) {
        socialLinks = req.body.social_links;
    }
    if (socialLinks) {
        socialLinks = Object.fromEntries(Object.entries(socialLinks).filter(([, value]) => Boolean(value)));
    }
    const logoFile = req.files?.logo?.[0];
    const faviconFile = req.files?.favicon?.[0];
    const settings = await (0, settings_service_1.upsertSettings)({
        site_name: req.body.site_name,
        footer_text: req.body.footer_text,
        contact_email: req.body.contact_email,
        contact_phone: req.body.contact_phone,
        social_links: socialLinks,
        logo_file: logoFile,
        favicon_file: faviconFile
    });
    res.status(200).json({ success: true, data: { settings } });
};
exports.updateWebsiteSettings = updateWebsiteSettings;
