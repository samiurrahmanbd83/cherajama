...
const buildAssetUrl = (file) => {
    if (!file)
        return undefined;
    return `${uploadsPublicBase}/${path_1.default.basename(file.path)}`;
};
// Fetch the latest website settings
const getSettings = async () => {
    const result = await pool_1.db.query(`SELECT id, site_name, logo_url, favicon_url, footer_text, contact_email, contact_phone,
...
const upsertSettings = async (input) => {
    ...
    if (!existing) {
        const inserted = await pool_1.db.query(`INSERT INTO website_settings
       ...
       RETURNING id, site_name, logo_url, favicon_url, footer_text, contact_email, contact_phone,
                 social_links, created_at, updated_at`, [siteName, logoUrl, faviconUrl, footerText, contactEmail, contactPhone, socialLinks]);
        return inserted.rows[0];
    }
    const updated = await pool_1.db.query(`UPDATE website_settings
     SET site_name = $1,
         logo_url = $2,
         favicon_url = $3,
         footer_text = $4,
